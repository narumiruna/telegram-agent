...
export function withDeadline<T>(deadlineAt: number | undefined, operation: () => Promise<T>): Promise<T> {
  return execution.run(mergedContext({ deadlineAt }), operation);
}

export function withAttemptSink<T>(attempts: AttemptRecord[], operation: () => Promise<T>): Promise<T> {
