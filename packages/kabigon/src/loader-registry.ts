...
export const LOADER_DEFS: readonly LoaderDef[] = [
  ...
  definition(REEL, "Instagram Reels audio transcription + metadata", "social_post", { resourceKind: "worker" }),
  ...
  definition(PLAYWRIGHT_NETWORKIDLE, "Browser-based scraping with networkidle wait", "generic_web", {
    cliVisible: false,
  ...
  definition(PLAYWRIGHT, "Browser-based scraping for any website", "generic_web", { resourceKind: "browser" }),
  ...
  definition(FIRECRAWL, "Firecrawl-based web extraction (requires FIRECRAWL_API_KEY)", "generic_web", {
    requirements: ["FIRECRAWL_API_KEY"],
    resourceKind: "worker",
  }),
  definition(YTDLP, "Audio transcription via yt-dlp + Whisper", "generic_web", { resourceKind: "worker" }),
];
