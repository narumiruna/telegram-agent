...
export class LoadChain {
  ...
  private async executeDetailed(attempts: AttemptRecord[], signal?: AbortSignal): Promise<LoadResult> {
    const errors: string[] = [];
    for (const plannedLoaderName of this.explanation.executionPlan) {
      ...
      try {
        ...
        const result = await runWithDeadline(
          this.admit ? () => this.admit?.(plannedLoaderName, operation) as Promise<string> : operation,
          remaining,
          signal,
        );
        if (!result.trim()) {
        ...
        return {
          ...
          downgraded: this.explanation.contentType !== ContentType.GenericWeb && actualType === ContentType.GenericWeb,
          attempts: [...attempts],
        };
      } catch (error) {
        if (signal?.aborted) throw signal.reason;
        ...
        if (error instanceof LoaderTimeoutError) {
          ...
          appendAttempt(plannedLoaderName, AttemptStatus.Timeout, started, error, "Loader timed out");
          continue;
        }
        if (isDeadlineError(error)) {
          errors.push(`${plannedLoaderName}: Shared deadline expired`);
          appendAttempt(plannedLoaderName, AttemptStatus.Timeout, started, error, "Deadline expired");
          break;
        }
        if (error instanceof LoaderContentError) {
          errors.push(`${plannedLoaderName}: Content extraction failed - ${error.reason}`);
          appendAttempt(plannedLoaderName, AttemptStatus.Failed, started, error, "Content extraction failed");
          continue;
        }
...
function deadlineSignal(remaining: number | undefined, signal?: AbortSignal): AbortSignal | undefined {
...
function ensureAnyEligible(explanation: LoadChainExplanation, lookup: RequirementsLookup): void {
  if (explanation.executionPlan.some((name) => missingRequirements(lookup(name)).length === 0)) return;
...
export function resolveExplicitLoadChain(
  ...
  const getRequirements = options.getRequirements ?? (isDefaultFactory ? getLoaderRequirements : () => []);
  const getContentType =
    options.getContentType ?? (isDefaultFactory ? getLoaderContentType : () => ContentType.GenericWeb);
