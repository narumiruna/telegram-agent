...
export function renderFxTwitterPayload(payload: unknown, expectedStatusId: string, sourceUrl: string): string {
  const envelope = record(payload);
  const tweet = record(envelope?.tweet);
  if (!tweet || text(tweet.id) !== expectedStatusId) {
    throw new LoaderContentError("TwitterLoader", sourceUrl, "FxTwitter did not return the requested tweet");
  }
...
export class TwitterLoader implements Loader {
  ...
  private async loadViaFxTwitter(target: TwitterTarget, signal?: AbortSignal): Promise<string> {
    ...
    try {
      ...
      throw new LoaderContentError("TwitterLoader", target.url, `FxTwitter returned invalid JSON: ${String(error)}`);
    }
  }
  ...
  private async loadViaBrowser(target: TwitterTarget, signal?: AbortSignal): Promise<string> {
    ...
    const extractTweet = async (page: Page): Promise<string> => {
      for (const article of await page.locator("article").all()) {
        ...
        if (href && statusId(href) === target.statusId) return article.evaluate((element) => element.outerHTML);
      }
