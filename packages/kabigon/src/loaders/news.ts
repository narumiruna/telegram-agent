...
export class NewsArticleLoader implements Loader {
  ...
  constructor(
    ...
    this.validate = source === "bbc" ? parseBbcTarget : source === "cnn" ? parseCnnTarget : parseLtnTarget;
  ...
  async load(url: string, signal?: AbortSignal): Promise<string> {
    ...
    const transports: readonly [string, () => Promise<{ content: string; contentType: string }>][] = [
      [
        "httpx",
        () =>
          fetchHttpHtml(url, { headers: this.headers, resources: this.resources, signal, loaderName: this.loaderName }),
      ],
      [
        "curl-cffi",
        () =>
