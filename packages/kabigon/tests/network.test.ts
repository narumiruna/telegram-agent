...
import { assertPublicUrl, type PublicUrlResolver, readResponseBytes, safeFetch } from "../src/core/network.js";
...
describe("network safety", () => {
  ...
  it("validates every redirect before issuing the next request", async () => {
    ...
    await expect(safeFetch("https://public.example/start", {}, { fetchImplementation, resolve })).rejects.toThrow(
      "Private",
    );
    expect(fetchImplementation).toHaveBeenCalledTimes(1);
  });
